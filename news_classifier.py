...
with open('tdata.json') as json_data:
...
#summary_writer = tf.summary.FileWriter('log_stats')
...
# we can start to predict the results for each of the 5 categories
for s in sent.keys():
    print("File: %s - category :%s" % (s, categories[np.argmax(model.predict([get_tf_record(sent[s])]))]))
